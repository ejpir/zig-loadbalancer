...
pub fn build(b: *std.Build) !void {
    ...
    var macro_list = try std.ArrayList(MacroPair).initCapacity(b.allocator, 16);
    ...
    // Create a module for the C library (no Zig source file)
    const bearssl_mod = b.createModule(.{
        .target = target,
        .optimize = optimize,
        .link_libc = true,
    });
    bearssl_mod.pic = true;
    ...
    bearssl_mod.addCSourceFile(.{ .file = upstream.path("src/settings.c") });

    bearssl_mod.addIncludePath(upstream.path("src/"));
    bearssl_mod.addIncludePath(upstream.path("inc/"));
    ...
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/aead"), .files = aead_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/codec"), .files = codec_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/ec"), .files = ec_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/hash"), .files = hash_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/int"), .files = int_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/kdf"), .files = kdf_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/mac"), .files = mac_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/rand"), .files = rand_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/rsa"), .files = rsa_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/ssl"), .files = ssl_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/symcipher"), .files = symcipher_src, .flags = flags });
    bearssl_mod.addCSourceFiles(.{ .root = upstream.path("src/x509"), .files = x509_src, .flags = flags });
