...
comptime {
    // Allow any 0.15.x version >= 0.15.1
    const zig_version_ok = builtin.zig_version.major == min_zig_version.major and
        builtin.zig_version.minor == min_zig_version.minor and
        builtin.zig_version.patch >= min_zig_version.patch;
    if (!zig_version_ok) {
        @compileError(std.fmt.comptimePrint(
            "unsupported zig version: expected >= {}, found {}",
            .{ min_zig_version, builtin.zig_version },
        ));
    }
}
